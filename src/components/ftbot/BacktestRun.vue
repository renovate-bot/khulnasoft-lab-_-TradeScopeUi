...
function clickBacktest() {
  ...
  if (btStore.chainAI.enabled) {
    btPayload.chainaimodel = btStore.chainAI.model;
    if (btStore.chainAI.identifier !== '') {
      btPayload.chainai = { identifier: btStore.chainAI.identifier };
    }
  }
  ...
  <BCard :disabled="botStore.activeBot.backtestRunning">
    ...
    >
      ...
      <template v-if="botStore.activeBot.botApiVersion >= 2.22">
        <BFormGroup
          label-cols-sm="5"
          label="Enable ChainAI:"
          label-align-sm="right"
          label-for="enable-chainai"
          class="align-items-center"
        >
          <template #label>
            <div class="d-flex justify-content-center">
              <span class="me-2">Enable ChainAI:</span>
              <InfoBox
                hint="Assumes chainAI configuration is setup in the configuration, and the strategy is a chainAI strategy. Will fail if that's not the case."
              />
            </div>
          </template>
          <BFormCheckbox id="enable-chainai" v-model="btStore.chainAI.enabled"></BFormCheckbox>
        </BFormGroup>
        <BFormGroup
          v-if="btStore.chainAI.enabled"
          label-cols-sm="5"
          label="ChainAI identifier:"
          label-align-sm="right"
          label-for="chainai-identifier"
        >
          <BFormInput
            id="chainai-identifier"
            v-model="btStore.chainAI.identifier"
            placeholder="Use config default"
          ></BFormInput>
        </BFormGroup>
        <BFormGroup
          ...
          label="ChainAI Model"
          label-align-sm="right"
          label-for="chainai-model"
        >
          <ChainaiModelSelect
            id="chainai-model"
            v-model="btStore.chainAI.model"
          ></ChainaiModelSelect>
        </BFormGroup>
      </template>
