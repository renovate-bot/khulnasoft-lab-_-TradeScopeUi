...
export function createBotSubStore(botId: string, botName: string) {
  ...
  const useBotStore = defineStore(botId, {
    ...
    getters: {
      ...
      botName: (state) => state.botState?.bot_name || 'chaindrift',
    ...
    actions: {
      ...
      async getStrategyPlotConfig() {
        try {
          ...
          if (plotConfig.subplots === null) {
            ...
            // TODO: Remove this fix when fix in chaindrift is populated further.
            plotConfig.subplots = {};
          }
      ...
      async getChainAIModelList() {
        try {
          const { data } = await api.get<ChainAIModelListResult>('/chainaimodels');
          this.chainaiModelList = data.chainaimodels;
          return Promise.resolve(data);
        } catch (error) {
          console.error(error);
          return Promise.reject(error);
